...
class Generator(nn.Module):
    def __init__(self,use_bias=False,inplace=True):
        ...
        self.up1=nn.Sequential(
            nn.ConvTranspose2d(256,256,kernel_size=3,stride=2,padding=1,bias=self.use_bias),
            nn.BatchNorm2d(256,affine=True),
            nn.ReLU()
        )
        self.up2=nn.Sequential(
            nn.ConvTranspose2d(256,128,kernel_size=3,stride=2,padding=1,bias=self.use_bias),
            nn.BatchNorm2d(128,affine=True),
            nn.ReLU()
        )
        self.up3=nn.Sequential(
            nn.ConvTranspose2d(128,64,kernel_size=3,stride=2,padding=1,bias=self.use_bias),
            nn.BatchNorm2d(64,affine=True),
            nn.ReLU()
        )
        self.up4=nn.Sequential(
            nn.ConvTranspose2d(64,32,kernel_size=3,stride=2,padding=1,bias=self.use_bias),
            nn.BatchNorm2d(32,affine=True),
            nn.ReLU()
        )
        self.last=nn.Conv2d(32,3,kernel_size=3,stride=2,padding=1,bias=self.use_bias)
